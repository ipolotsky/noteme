...
def subscription_plans_kb(plans: list[SubscriptionPlan], lang: str) -> InlineKeyboardMarkup:
    ...
    for plan in plans:
        ...
        if plan.discount_percent:
            label += (
                f" {t('subscription.discount_label', lang, percent=str(plan.discount_percent))}"
            )
        rows.append(
            [
